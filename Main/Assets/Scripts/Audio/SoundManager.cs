...
public class SoundManager : MonoBehaviour
{
    ...
    public static SoundManager Instance
    {
        get
        {
            // Автоматически создаём SoundManager если его нет
            if (_instance == null)
            {
                ...
                if (_instance == null)
                {
                    ...
                    Debug.Log("SoundManager: Автоматически создан");
                }
            }
            return _instance;
        }
    ...
    private void Awake()
    {
        ...
        // Загружаем звуки из Resources если не назначены
        LoadSoundsFromResources();

        // Создаём пул аудио источников
        CreateAudioSourcePool();

        Debug.Log("SoundManager: Инициализирован");
    }
    ...
    private void LoadSoundsFromResources()
    {
        ...
        // Звуки бонусов/пикапов
        if (_pickupSound == null)
        {
            _pickupSound = Resources.Load<AudioClip>("Sounds/Bonuses_sound");
        }

        Debug.Log($"SoundManager: Загружены звуки - меч:{_swordHitSounds?.Length ?? 0}, бомба:{_bombExplosionSound != null}, пикап:{_pickupSound != null}");
    }
